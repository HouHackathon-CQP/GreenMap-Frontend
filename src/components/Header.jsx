...
export default function Header({ setIsSidebarOpen, onLogout }) {
  ...
  return (
    <header className="flex-shrink-0 flex items-center justify-between h-16 p-4 md:px-6 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700/50 relative z-20 transition-colors duration-300">
      ...
      <div className="hidden lg:block">
        <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100 tracking-tight">Hệ Thống Quản Trị Trung Tâm</h2>
      </div>
      ...
      <div className="flex items-center space-x-2 md:space-x-4">
        ...
        <div className="relative">
          <button onClick={() => setIsNotifOpen(!isNotifOpen)} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white relative p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
            <Bell size={20} />
          ...
          {isNotifOpen && (
             <>
               ...
               <div className="
                    z-50 bg-white dark:bg-[#111318] rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden 
               ...
               ">
                 <div className="p-4 border-b border-gray-200 dark:border-gray-700 font-bold text-gray-800 dark:text-white flex justify-between items-center">
                     <span>Thông báo</span>
                     <span className="text-xs bg-red-500/20 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-full">{notifications.length} mới</span>
                 </div>
                 <div className="max-h-80 overflow-y-auto custom-scrollbar">
                   {notifications.length === 0 ? (
                       ...
                       notifications.map(n => (
                         <div key={n.id} onClick={() => { navigate('/reports'); setIsNotifOpen(false); }} className="p-4 border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer transition-colors group">
                            <div className="flex justify-between items-start mb-1">
                                <span className="text-sm font-bold text-red-500 dark:text-red-400 group-hover:text-red-600 dark:group-hover:text-red-300 transition-colors">{n.title}</span>
                 ...
                 {notifications.length > 0 && (
                     <div onClick={() => { navigate('/reports'); setIsNotifOpen(false); }} className="p-3 text-center text-xs font-bold text-emerald-600 dark:text-emerald-500 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer border-t border-gray-200 dark:border-gray-700">
                         Xem tất cả báo cáo
                     </div>
                 )}
               </div>
          ...
          {isUserMenuOpen && (
            <>
              ...
              <div className="absolute right-0 mt-2 w-60 bg-white dark:bg-[#111318] rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
                ...
                <ul className="py-1.5">
                  <li>
                    <button onClick={() => { navigate('/settings'); setIsUserMenuOpen(false); }} className="flex items-center w-full px-4 py-2.5 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white transition-colors">
                        <Settings size={16} className="mr-3 text-gray-400 dark:text-gray-500"/> Cài đặt tài khoản
                    </button>
                  </li>
                  <li className="border-t border-gray-100 dark:border-gray-800 mt-1 pt-1">
                    <button onClick={onLogout} className="flex items-center w-full px-4 py-2.5 text-sm text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors">
