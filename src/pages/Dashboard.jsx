...
import { 
  Radio, Signal, AlertCircle, Activity, BarChart3, MapPin, 
  X, Wind, Thermometer, Droplets, Filter 
} from 'lucide-react';
...
// --- 1. DANH SÁCH TRẠM QUAN TRỌNG (WHITELIST) ---
// Chỉ hiển thị những trạm này khi bật chế độ Lọc
...
// Hàm kiểm tra: Trạm này có phải là trạm mục tiêu không?
const isTargetStation = (stationName) => {
  if (!stationName) return false;
  return TARGET_STATIONS.some(target => 
...
const StationDetailModal = ({ station, onClose }) => {
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300 cursor-pointer">
            ...
            <div className="p-6 relative z-10 overflow-y-auto custom-scrollbar">
                ...
                <div className="text-center">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-6">
                        <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-200 dark:border-gray-700"><div className="text-xs text-gray-500 dark:text-gray-400 mb-1 flex justify-center items-center"><Thermometer size={12} className="mr-1"/> Nhiệt độ</div><div className="font-bold text-lg text-gray-900 dark:text-white">{station.temperature}°</div></div>
                        <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-200 dark:border-gray-700"><div className="text-xs text-gray-500 dark:text-gray-400 mb-1 flex justify-center items-center"><Droplets size={12} className="mr-1"/> Độ ẩm</div><div className="font-bold text-lg text-gray-900 dark:text-white">{station.humidity}%</div></div>
                        <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-200 dark:border-gray-700"><div className="text-xs text-gray-500 dark:text-gray-400 mb-1 flex justify-center items-center"><Wind size={12} className="mr-1"/> Gió</div><div className="font-bold text-lg text-gray-900 dark:text-white">{station.wind_speed} <span className="text-xs">m/s</span></div></div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700/50 rounded-2xl p-4 mb-4 text-left">
                        <div className="flex justify-between items-center mb-4"><h4 className="text-sm font-bold text-gray-700 dark:text-gray-300 flex items-center"><Activity size={14} className="mr-2 text-yellow-500"/> Dự báo 24h tới</h4>{localForecast?.current && <span className="text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-400/10 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-400/20">{localForecast.current.desc}</span>}</div>
                        <div className="h-[150px] w-full min-w-0">
                            {loadingForecast ? ( <div className="h-full flex items-center justify-center text-gray-500 text-xs"><Wind className="animate-spin mr-2" size={16}/> Đang tải...</div> ) : localForecast && localForecast.hourly_24h ? (
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
            <button 
                ...
                title={isFiltered ? "Bấm để hiện tất cả các trạm" : "Bấm để chỉ xem các trạm chính"}
                className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-sm font-bold shadow-sm transition-all active:scale-95 border ${
                    isFiltered 
            ...
            >
                <Filter size={16} className={isFiltered ? "fill-white" : ""}/>
                <span>{isFiltered ? 'Đã lọc trạm' : 'Tất cả trạm'}</span>
            </button>
        ...
        <div className="bg-white dark:bg-[#111318] rounded-3xl border border-gray-200 dark:border-gray-800 shadow-xl dark:shadow-none flex flex-col overflow-hidden h-[400px] lg:h-full transition-colors duration-300">
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center">
                <h3 className="text-gray-900 dark:text-white font-bold text-base flex items-center gap-2">
                    Danh sách trạm <span className="bg-gray-200 dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-[10px] px-2 py-0.5 rounded-full">{displayedSensors.length}</span>
                </h3>
            </div>

            <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
                {displayedSensors.map((s, i) => (
                    <div key={i} onClick={() => setSelectedStation(s)} className="flex justify-between p-3 bg-gray-50 dark:bg-gray-800/30 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl cursor-pointer border border-transparent hover:border-gray-300 dark:hover:border-gray-700 transition-colors group">
                        <div className="truncate pr-2">
                            <h4 className="text-gray-800 dark:text-gray-200 text-sm font-bold truncate group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{s.station_name}</h4>
                            <p className="text-[10px] text-gray-500 truncate">{s.provider || 'N/A'}</p>
                        </div>
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm shadow-sm ${getAQIInfo(s.value).bg} ${getAQIInfo(s.value).text}`}>
                            {s.value}
                        </div>
                    </div>
                ))}
