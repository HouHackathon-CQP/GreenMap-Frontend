...
import { Radio, Signal, AlertCircle, Loader2, X, MapPin, Activity, BarChart3, Wind, Droplets, Thermometer } from 'lucide-react';
...
import { getAQIInfo } from '../utils/aqiCalculator'; // Import logic màu mới
...
// --- MODAL CHI TIẾT (Giao diện cũ gọn gàng) ---
const StationDetailModal = ({ station, onClose }) => {
  if (!station) return null;
  const info = getAQIInfo(station.value); // Dùng logic màu mới
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in zoom-in duration-200 cursor-pointer">
        <div onClick={(e) => e.stopPropagation()} className="bg-[#111318] border border-gray-700/60 rounded-3xl w-full max-w-lg shadow-2xl relative overflow-hidden cursor-default">
            {/* Glow Effect */}
            ...
            <div className="p-6 relative z-10">
                <div className="flex justify-between items-start mb-6">
                    <div>
                        <h2 className="text-2xl font-bold text-white tracking-tight">{station.station_name}</h2>
                ...
                <div className={`flex items-center justify-between p-5 rounded-2xl border ${info.border} ${info.bg} mb-6 backdrop-blur-xl`}>
                    <div>
                        <p className="text-xs font-bold uppercase tracking-widest text-gray-300 mb-1">Chỉ số AQI</p>
                        <div className="flex items-baseline gap-2">
                            <span className={`text-6xl font-black ${info.text} tracking-tighter`}>{station.value ?? '--'}</span>
                            <span className="text-sm font-medium text-gray-400">{info.level}</span>
                        </div>
                    </div>
      ...
      try {
        ...
        if (aqiResult && Array.isArray(aqiResult.data)) {
          aqiResult.data.forEach(s => {
            const val = s.value; // Đây đã là AQI rồi
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-[#111318] border border-gray-800 p-6 rounded-2xl relative overflow-hidden group hover:border-emerald-500/50 transition-all shadow-lg">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center space-x-1 bg-emerald-500/10 px-2 py-1 rounded border border-emerald-500/20"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span><span className="text-xs font-bold text-emerald-500">Online</span></div>
            </div>
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 p-6 rounded-2xl relative overflow-hidden shadow-lg">
            ...
            <div className="text-4xl font-black text-white mb-1">{kpiData.avgAqi} <span className="text-sm font-medium text-gray-400">AQI</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-[600px]">
        <div className="lg:col-span-2 bg-[#111318] rounded-3xl border border-gray-800 overflow-hidden shadow-xl relative flex flex-col h-[400px] lg:h-auto">
            <div className="absolute top-0 left-0 w-full z-10 p-5 bg-gradient-to-b from-black/80 to-transparent pointer-events-none flex justify-between items-start">
                <div><h3 className="text-white font-bold text-lg flex items-center"><MapPin className="mr-2 text-emerald-500" size={20}/> Bản đồ Trực quan</h3></div>
            ...
            <div className="flex-1 w-full bg-gray-900"><GreenMap onStationSelect={setSelectedStation} /></div>
        </div>
        ...
        <div className="bg-[#111318] rounded-3xl border border-gray-800 shadow-xl flex flex-col overflow-hidden h-[400px] lg:h-auto">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2 bg-gradient-to-b from-[#111318] to-[#0f1014]">
                {isLoading ? (
                    <div className="h-full flex flex-col items-center justify-center text-emerald-500"><Loader2 size={40} className="animate-spin mb-3"/><span className="text-xs uppercase tracking-widest font-bold">Đang đồng bộ...</span></div>
                ) : sensorList.map((s, i) => {
                    const info = getAQIInfo(s.value);
                    return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#111318] border border-gray-800 p-6 rounded-3xl shadow-lg">
            <div className="flex justify-between items-center mb-6"><h3 className="text-white font-bold text-lg flex items-center"><Activity className="text-orange-500 mr-2"/> Xu hướng 24h</h3></div>
            <div className="h-[300px] w-full min-h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={trendData}>
                        <defs><linearGradient id="colorTrend" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f97316" stopOpacity={0.3}/><stop offset="95%" stopColor="#f97316" stopOpacity={0}/></linearGradient></defs>
                        ...
                        <Tooltip contentStyle={{backgroundColor:'#111827', border:'1px solid #374151', borderRadius:'12px', boxShadow:'0 10px 15px rgba(0,0,0,0.5)'}} itemStyle={{color:'#fff', fontWeight:'bold'}} labelStyle={{color:'#9ca3af', fontSize:'12px', marginBottom:'5px'}}/>
        ...
        <div className="bg-[#111318] border border-gray-800 p-6 rounded-3xl shadow-lg">
            <div className="flex justify-between items-center mb-6"><h3 className="text-white font-bold text-lg flex items-center"><BarChart3 className="text-blue-500 mr-2"/> Ô nhiễm theo Quận (Top 5)</h3></div>
            <div className="h-[300px] w-full min-h-[300px]">
                {districtData.length === 0 ? <div className="h-full flex items-center justify-center text-gray-600 italic font-medium">Chưa đủ dữ liệu phân tích</div> : (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={districtData} layout="vertical" margin={{left: 0}}>
                            ...
                            <Tooltip cursor={{fill: '#374151', opacity: 0.2}} contentStyle={{backgroundColor:'#111827', border:'1px solid #374151', borderRadius:'12px', boxShadow:'0 10px 15px rgba(0,0,0,0.5)'}} itemStyle={{color:'#fff', fontWeight:'bold'}} labelStyle={{color:'#9ca3af', fontSize:'12px', marginBottom:'5px'}}/>
